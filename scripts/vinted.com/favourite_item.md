...
Automatically favourite an item on vinted.com. Add a Vinted listing to the signed-in account's favourites. Does nothing if it is already favourited, and confirms the result by reading the listing's own favourite state back after the change.

- Site: vinted.com
- Address: `reduck/vinted.com/favourite_item`
- Updated: 2026-08-14 (v1)
- Author: Reduck AI (reduck)

## Run it

Ask an AI agent connected to Reduck over MCP (`https://mcp.reduck.ai/mcp`) to run `reduck/vinted.com/favourite_item`, or run it from a terminal:

```bash
npx @reduck-ai/cli@latest run --script reduck/vinted.com/favourite_item
```
...
- `id` (string, required)
- `url` (string, required)
- `favourited` (boolean, required): The listing's favourite state read back from the page after the run.
...
- `already_favourited` (boolean, optional): True when it was already a favourite and nothing was changed.

## FAQ

### What does "Vinted — favourite an item" do?
...
### How do I automatically favourite an item on vinted.com?
...
### Is there a vinted.com API to favourite an item?

You do not need one. "Vinted — favourite an item" drives the real vinted.com pages in a browser, so it works whether or not vinted.com offers an API for this.
...
Source: https://reduck.ai/explore/scripts/reduck/vinted.com/favourite_item
